...
public class VedleggEksempel {
	...
	public static void main(final String[] args) {
		...
		Message message = new Message(UUID.randomUUID().toString(), "Brevets emne", pin, new SmsNotification(), PASSWORD, NORMAL, PDF);

		// 5. Vi henter inputstreamen til filen vi ønsker å sende
		InputStream messageContent = getMessageContent();

		// 6. Vi lar klientbiblioteket håndtere å legge til innhold i hoveddokumentet
		...
		// 9. Vi lar klientbiblioteket håndtere å legge til innhold i vedlegget
		MessageDelivery messageWithAttachment = client.createAttachment(messageWithContent, attachment, attachmentContent);

		// 10. Vi lar klientbiblioteket håndtere sending av forsendelsen
		client.sendMessage(messageWithAttachment);
	}
