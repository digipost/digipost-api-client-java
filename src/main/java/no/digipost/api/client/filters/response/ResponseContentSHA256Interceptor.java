...
public class ResponseContentSHA256Interceptor implements HttpResponseInterceptor {
	...
	public void process(HttpResponse response, HttpContext context) {
		...
		try {
			...
			if (entity != null && entity.getContent() != null) {
				...
				if (entityBytes.length > 0) {
					...
					if (isBlank(hashHeader)) {
						throwOrWarn(SERVER_SIGNATURE_ERROR, "Mangler X-Content-SHA256-header - server-signatur kunne ikke valideres");
					}
			...
			throwOrWarn(SERVER_SIGNATURE_ERROR, "Det skjedde en feil under uthenting av innhold for validering av X-Content-SHA256-header - server-signatur kunne ikke valideres: " + e.getMessage());
		}
	}
	...
	private void validerBytesMotHashHeader(final String serverHash, final byte[] entityBytes, HttpResponse response) {
		...
		if (!serverHash.equals(ourHash)) {
			throwOrWarn(SERVER_SIGNATURE_ERROR, "X-Content-SHA256-header matchet ikke innholdet - server-signatur er feil.");
		}
	}


	private void throwOrWarn(ErrorCode e, String message) {
		if(shouldThrow) {
			throw new DigipostClientException(e, message);
		} else {
			LOG.warn(message);
		}
	}
